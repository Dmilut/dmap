...
public class NewMapScene extends AbstractMapScene {
	...
	private void restart() {
		...
		primaryStage.setTitle("ОРВД: " + mData.getTitle()); // "ОРВД - Планшет
															// Инженера");
		// primaryStage.show();
	}
	...
	private void setActions() {
		 ...
		 * mouseDown.set(mousePress); //System.out.println("press @ x=" +
		 * mousePress.getX()+" y=" + mousePress.getY()); });
		 * 
		 * imageView.setOnMouseDragged(e -> { System.out.println("drag @ e=" +
		 * e); Point2D dragPoint = imageViewToImage(imageView, new
		 * Point2D(e.getX(), e.getY())); shift(imageView,
		...
		imageView.setOnMouseMoved(e -> {
			...
			// System.out.println("move @ x=" + mouseClick.getX()+" y=" +
			// mouseClick.getY());
	...
	private void setZoom(ScrollEvent e) {
		...
		double scale = clamp(Math.pow(1.01, delta),
				...
				// don't scale so we're zoomed in to fewer than MIN_PIXELS in
				// any direction:
		...
		// imageView.setViewport(new Rectangle2D(newMinX, newMinY, newWidth,
		// newHeight));
	 ...
	 * 
	 * double width = imageView.getImage().getWidth() ; double height =
	 * imageView.getImage().getHeight() ;
	 * 
	 * double maxX = width - viewport.getWidth(); double maxY = height -
	 * viewport.getHeight();
	 * 
	 * double minX = clamp(viewport.getMinX() - delta.getX(), 0, maxX); double
	 * minY = clamp(viewport.getMinY() - delta.getY(), 0, maxY);
	 * 
	 ...
	 * imageViewCoordinates.getX() / imageView.getBoundsInLocal().getWidth();
	...
	private Point2D imageViewToImage(ScrollPane imageView, Point2D imageViewCoordinates) {
		...
		// Bounds viewport = imagePane.getViewportBounds();
		Point2D out = new Point2D(imageViewCoordinates.getX() - viewport.getMinX(),
		 ...
		 * imageView.getBoundsInLocal().getHeight();
		 * 
		 * //Rectangle2D viewport = imageView.getViewport(); Bounds viewport =
